...
export type DomainFindAll<T extends (BasicStoreEntity & { filters: string[], orderBy: string })> = (context: AuthContext, user: AuthUser, args: EntityOptions<T>) => Promise<StoreEntityConnection<T>>;
export type DomainFindById<T extends BasicStoreObject> = (context: AuthContext, user: AuthUser, id: string, args?: any) => Promise<T>;
